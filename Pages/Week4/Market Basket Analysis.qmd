...
-   *Application of market–basket analysis on healthcare by Rao et al. 2020* ([view paper](Papers/Rao et al_2020_Overview Market–Basket Analysis on Healthcare.pdf))

Examples of different applications:

-   *Correlates of Nonrandom Patterns of Serotype Switching in Pneumococcus by Joshi et al. 2019* ([view paper](Papers/Joshi et al_2019_Nonrandom Patterns of Serotype Switching in Pneumococcus.pdf))
-   *Applying Market Basket Analysis to Determine Complex Coassociations Among Food Allergens in Children With Food Protein-Induced Enterocolitis Syndrome (FPIES) by Banerjee et al. 2024* ([view paper](Papers/Banerjee et al_2024_Coassociations Among Food Allergens in Children With Food Protein-Induced Enterocolitis Syndrome (FPIES).pdf))
...
<div style="display: flex; justify-content: center; width: 100%;">
  ...
  <div style="flex: 1; margin-left: 10px; text-align: center; position: relative;">
    <h3>Interactive htmlwidget Plot</h3>
    <iframe id="htmlwidget_plot_frame2" src="pneumo_rules_plot.html" width="100%" height="500px" style="border: 1px solid #ccc;"></iframe>
    <div style="position: absolute; bottom: 10px; right: 10px;">
      <button onclick="zoomIn('htmlwidget_plot_frame2')">Zoom In</button>
      <button onclick="zoomOut('htmlwidget_plot_frame2')">Zoom Out</button>
      <button onclick="resetHtmlwidgetPlot()">Reset</button>
    </div>
  </div>
</div>

<script>
  let zoomLevelHtmlwidget = 1; // Initial zoom level for htmlwidget
  const zoomFactorHtmlwidget = 1.1;
  let initialStateHtmlwidget = {
    transform: '',
    scrollPosition: { left: 0, top: 0 }
  };

  function centerGraphContainerHtmlwidget(iframeDocument, graphContainer) {
    const iframeWindow = iframeDocument.defaultView;
    const scaledWidth = graphContainer.scrollWidth * zoomLevelHtmlwidget;
    const scaledHeight = graphContainer.scrollHeight * zoomLevelHtmlwidget;
    const widthScroll = (scaledWidth - iframeWindow.innerWidth) / 2;
    const heightScroll = (scaledHeight - iframeWindow.innerHeight) / 2;
    iframeWindow.scrollTo(widthScroll, heightScroll);
  }

  function applyZoomAndCenterHtmlwidget(frameId) {
    const frame = document.getElementById(frameId);
    if (frame && frame.contentWindow) {
      const iframeDocument = frame.contentDocument || frame.contentWindow.document;
      const graphContainer = iframeDocument.querySelector('.vis-network');
      if (graphContainer) {
        graphContainer.style.transform = `scale(${zoomLevelHtmlwidget})`;
        graphContainer.style.transformOrigin = '0 0';
        centerGraphContainerHtmlwidget(iframeDocument, graphContainer);
      }
    }
  }

  function zoomIn(frameId) {
    zoomLevelHtmlwidget *= zoomFactorHtmlwidget;
    applyZoomAndCenterHtmlwidget(frameId);
  }
  ...
  function captureInitialViewHtmlwidget() {
    const frame = document.getElementById('htmlwidget_plot_frame2');
    const iframeDocument = frame.contentDocument || frame.contentWindow.document;
    const iframeWindow = frame.contentWindow;
    ...
    const graphContainer = iframeDocument.querySelector('.vis-network');
    if (graphContainer) {
      initialStateHtmlwidget.transform = graphContainer.style.transform;
      initialStateHtmlwidget.scrollPosition.left = iframeWindow.scrollX;
      initialStateHtmlwidget.scrollPosition.top = iframeWindow.scrollY;

      console.log('Captured Initial State:', initialStateHtmlwidget);
    }
  }

  function restoreInitialViewHtmlwidget() {
    ...
    const graphContainer = iframeDocument.querySelector('.vis-network');
    if (graphContainer) {
      graphContainer.style.transform = initialStateHtmlwidget.transform;
      iframeWindow.scrollTo(initialStateHtmlwidget.scrollPosition.left, initialStateHtmlwidget.scrollPosition.top);

      console.log('Restored Initial State:', initialStateHtmlwidget);
    }
  }

  function initialViewHtmlwidget() {
    const frame = document.getElementById('htmlwidget_plot_frame2');
    const iframeDocument = frame.contentDocument || frame.contentWindow.document;
    const iframeWindow = frame.contentWindow;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const frame = document.getElementById('htmlwidget_plot_frame2');
    frame.onload = captureInitialViewHtmlwidget;
    initialViewHtmlwidget(); // Ensure initial centering
  });

  window.addEventListener('resize', () => {
    initialViewHtmlwidget();
  });

  function resetHtmlwidgetPlot() {
    const frame = document.getElementById('htmlwidget_plot_frame2');
    frame.onload = () => {
      restoreInitialViewHtmlwidget();
    };
    frame.src = frame.src + "?reload=" + new Date().getTime(); // Force reload to get the iframe back to its initial state
  }
</script>
```

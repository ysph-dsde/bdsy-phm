...
This workshop was developed by [Dr. Daniel Weinberger](https://ysph.yale.edu/profile/daniel-weinberger/) from the Yale School of Public Health (YSPH) for the 12$^{th}$ International Symposium on Pneumococci and Pneumococcal Diseases (ISPPD-12) hosted in Toronto, Canada in June 2022. Here, we have adapted his workshop session #1, which compares different methods for fitting a time series dataset\ [@isppd_github]. Students are encouraged to explore the workshop content that Dr. Weinberger put together to learn more about the subject:

-   [GitHub:](https://github.com/DanWeinberger/isppd_workshop2022#) Find the code, original data, and slides\ [@isppd_github].
-   [Workshop Webpage:](https://vaccineevaluationworkshop.github.io/#content) Find a longer description of the workshop in the format of a mini course with links to video presentations of select sections - *Workshop on the evaluation of vaccine impact*\ [@isppd_webpage].
...
To control for temporal variations such as seasonality, we can add a term for seasonal adjustment in the generalized linear model. By incorporating variables representing different time subsections as new regressors into the simple linear regression model, we allow for changes in the mean value of the underlying regression relationship to be influenced by the time of the year.

::: {.callout-tip collapse="true"}
## Models Sensitive to Time Variance with Dynamic Harmonic Regression

There are advanced methods for detecting and modeling the impact of non-stationary periodic or quasi-periodic phenomena using stochastic techniques such as Dynamic Harmonic Regression (DHR), where model parameters vary over time. DHR is one form of the Unobserved Component (UC) model, which is useful for smoothing and signal extraction in complex time-series analysis\ [@Young2011].

Below is a basic form of a DHR model equation from Chapter 5.2 in _Recursive Estimation and Time-Series Analysis_ by Young\ [@Young2011].

\begin{align*}
y(k) = T(k) + C(k) + S(k) + \epsilon(k) \tag{$\epsilon(k) \sim \mathcal{N}\left(0, \sigma^2\right)$}
\end{align*}

where $T(k)$ is a trend or low-frequency component, and seasonal $S(k)$ and cyclical $C(k)$ components are kept separate due to potentially different physical interpretations. They have the following harmonic expressions

\begin{align*}
S(k) &= \sum_{i=1}^{R_S}\ \left\{a(i, k) \cos(\omega_i\ k) + b(i, k) \sin(\omega_i\ k)\right\}\\
C(k) &= \sum_{i=1}^{R_C}\ \left\{a(i, k) \cos(f_i\ k) + b(i, k) \sin(f_i\ k)\right\}
\end{align*}

where $a(i, k)$ and $b(i, k)$ are the stochastic time-varying parameters, $\omega_i$ for $i = 1, 2, \dots, R_S$ are the fundamental and harmonic frequencies associated with seasonality, and $f_i$ for $i = 1, 2, \dots, R_C$ are the frequencies associated with the cyclical component.
:::

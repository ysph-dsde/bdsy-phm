...
Consider the following Binomial-Poisson hierarchy example from _Statistical Inference_ by Casella and Berger (example 4.4.1)\ [@Casella2002].
...
&= \frac{(\lambda p)^x e^{-\lambda}}{x!}\ e^{(1-p)\lambda} \tag{sum a power series is exponential}\\
...
Using theorem 4.4.3 we get the mean of $X$ to be:

...
In this section we are going to test whether the trend or level of pneumoia as a result of pneumococcal disease changes after the introduction of the PCV10 vaccine. We can use three modeling methods to support a statistically informed answer to this question:
...
# View the changes.
df[c(1:5, 100:105), c("index", "date", "period1", "period2")]
```
...
mod_method1 <- glm.nb(J12_J18_prim ~ index + month + offset(log.offset) +
...
# Make predictions with confidence intervals.
pred4 <- predict(mod_method1, type = "response", se.fit = TRUE)
...
df.pred.its$pred4.cf <- predict(mod_method1, type = "response", 
